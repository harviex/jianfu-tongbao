...
class DailyProcessor:
    ...
    def _build_batch_prompt(self, batch: List[dict]) -> str:
        ...
        # 2026-08-29 改写：删除"核心判定逻辑"定义段，改为负面清单+反模板套话，避免模型 reward hacking。
        return "<think:6124c78e>\n</think:6124c78e>" + f"""你是"形式主义为基层减负"专项线索识别专家。请逐条判读以下 {len(batch)} 条 12345 诉求。
...
{chr(10).join(items)}"""
    ...
    def review_filter(self):
        ...
        NEGATIVE_PATTERNS = [
            ...
            r'仅属(?:于)?(?:个别|正常|技术|系统)',
            # 新增：模型实际输出的否定表述
            r'不符合[「"\'\'](?:形式主义|为基层增加负担|为基层减负|形式主义为基层增加负担)',
            r'不满足[「"\'\'](?:形式主义|为基层增加负担|为基层减负|形式主义为基层增加负担)',
            r'不构成[因]*形式主义',
            r'不属于[「"\'\'](?:形式主义|为基层增加负担|为基层减负)',
            r'未发现[「"\'\'](?:形式主义|为基层增加负担|为基层减负)',
            r'未体现[「"\'\'](?:形式主义|为基层增加负担|为基层减负)',
            r'仅为(?:技术|系统|业务|正常)',
            r'仅属于(?:技术|系统|业务|正常)',
            r'仅属(?:技术|系统|业务|正常)',
            r'属于正常工作',
